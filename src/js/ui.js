...
function renderKnockoutListView() {
    ...
    // Lista detalhada (cards completos) — distinta da chave clássica com mini-cards
    return `
        <div class="bk-cascade ko-phase-focus ko-list-view">
            <div class="bk-phase" data-phase-key="${phaseKey}" style="--bk-accent: ${accent}">
                <div class="bk-phase-header">
                    ...
                    <div class="bk-phase-heading">
                        <h4 class="bk-phase-title">${localizedFase}</h4>
                        <p class="bk-phase-sub">Lista detalhada · placares, sede e status</p>
                    </div>
                ...
                <div class="ko-list-stack">
                    ${matches.map((match) => renderKnockoutMatchCard(match, { phaseKey, compact: false })).join('')}
                </div>
            </div>
        </div>
    `;
...
function renderBracketColumn(options = {}) {
    ...
    if (side === 'center') {
        return `<div class="${baseClass}${focusClass}" data-side="center"></div>`;
    }
...
/**
 * Chave clássica "Road to the Trophy":
...
function renderKnockoutBracketView() {
    ...
    // Bracket tree: esquerda (caminho A) → troféu → direita (caminho B)
    const columns = [
